...
class UserTable(alias: String) extends Table[User, MutableUser]("example", "user", alias, false, true) with JSON4SJsonFields[User, MutableUser] {
  ...
  def _newRecordFromResultSet($rs: ResultSet, $i: Int): User = new User(id.getTableValue($rs, $i), email.getTableValue($rs, $i), passwordHash.getTableValue($rs, $i), active.getTableValue($rs, $i), rating.getTableValue($rs, $i), verbose.getTableValue($rs, $i), jsB.getTableValue($rs, $i))
}
...
class User(val id: Int,
           ...
           val jsB: JValue) extends TableRecord {
  def _table = User
  def _primaryKey: Int = id
  def toMutable: MutableUser = { val m = new MutableUser; m.id = id; m.email = email; m.passwordHash = passwordHash; m.active = active; m.rating = rating; m.verbose = verbose; m.jsB = jsB; m }
}
...
class MutableUser extends MutableTableRecord[User] {
  ...
  def toRecord: User = new User(id, email, passwordHash, active, rating, verbose, jsB)
}
