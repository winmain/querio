...
object ExampleQueries {
  ...
  def queryAllUsers: Vector[User] = Db.queryAll(User)
  ...
  def addNewUserWithLog(md: ModifyData)(email: String, passwordHash: String): User = {
    Db.dataTrReadUncommitted(md) {implicit dt =>
      val user = new MutableUser
      user.email = email
      user.passwordHash = passwordHash
      user.rating = Some(1)
  ...
  def deleteAll() = {
    Db.dataTrReadCommittedNoLog {implicit dt =>
      val allUsers: Vector[User] = Db.queryAll(User)
      val allUserIDs = allUsers.map(_.id)
      allUserIDs.foreach(Db.deleteRaw(User, _))
    }
  }
}
